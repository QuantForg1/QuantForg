...
# Same stop geometry as RiskEngine ATR path (atr x multiplier).
...
def stop_distance_from_atr(
    ...
    *,
    ...
    """Institutional-compatible stop distance: ATR x multiplier."""
    if atr <= 0:
...
def dollar_risk_at_lots(
    *,
    ...
    """Exact loss if stop is hit: lots x stop x contract_size."""
...
def evaluate_balance(
    ...
    *,
    ...
    tradability = classify_tradability(required_risk_pct=required_pct, profile=profile)
    ...
    if tradability is MicroTradability.NOT_TRADABLE:
        ...
        recommendations.append(
            ...
            "would cut dollar risk ~10x at the same stop."
        )
        recommendations.append(
        ...
        recommendations.append(
            ...
            f"(need >= ${equity_floor_for_risk(dollar_risk_at_min_lot=max_loss, risk_pct=profile.recommended_max_risk_pct)})."  # noqa: E501
        )
        recommendations.append(
