"""File-backed persistence for Live Trading Evidence Program."""
...
def data_path(filename: str) -> Path:
    try:
        ...
        base = Path("data")
    return base / "live_trading_evidence" / filename
        ...
        with self._lock:
            for row in self._docs:
                if str(row.get("id")) == str(doc_id):
                    return deepcopy(row)
                # Also match trade_id / ticket aliases
                if str(row.get("trade_id") or "") == str(doc_id):
                    return deepcopy(row)
                if str(row.get("mt5_ticket") or "") == str(doc_id):
                    return deepcopy(row)
                if str(row.get("validation_id") or "") == str(doc_id):
                    return deepcopy(row)
        return None
