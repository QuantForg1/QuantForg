...
class ProductionReadinessCertification:
    ...
    def evaluate(self, inp: PrcInput) -> dict[str, Any]:
        ...
        if flags.get("risk_certification", True):
            modules["risk_certification"] = risk_certification(inp, self.config)
        ...
        if flags.get("decision_certification", True):
            modules["decision_certification"] = decision_certification(inp, self.config)
        if flags.get("data_certification", True):
            modules["data_certification"] = data_certification(inp, self.config)
        if flags.get("research_certification", True):
            modules["research_certification"] = research_certification(inp, self.config)
        ...
        if flags.get("human_signoff_package", True):
            modules["human_signoff_package"] = human_signoff_package(dict(modules))

        dash = modules.get("readiness_dashboard")
        current_status = str(
            (dash.details or {}).get("certification_status") if dash else INSUFFICIENT
        )
        ...
        if flags.get("continuous_certification", True):
            ...
            entry = (modules["continuous_certification"].details or {}).get("entry")
        ...
        return {
            ...
            "certification_report": {
                ...
                "certification_decision": (
                    (hsp.details or {}).get("certification_decision") if hsp else None
                ),
                "known_risks": ((hsp.details or {}).get("known_risks") if hsp else []),
                "open_issues": ((hsp.details or {}).get("open_issues") if hsp else []),
...
def input_from_dict(data: dict[str, Any]) -> PrcInput:
    return PrcInput(
        ...
        execution=(
            data.get("execution") if isinstance(data.get("execution"), dict) else None
        ),
        decision=(
            data.get("decision") if isinstance(data.get("decision"), dict) else None
        ),
        data=(data.get("data") if isinstance(data.get("data"), dict) else None),
        research=(
            data.get("research") if isinstance(data.get("research"), dict) else None
        ),
        operations=(
            data.get("operations") if isinstance(data.get("operations"), dict) else None
        ),
