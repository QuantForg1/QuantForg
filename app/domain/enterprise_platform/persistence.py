"""Enterprise Platform persistence helpers."""
...
def data_path(filename: str) -> Path:
    try:
        ...
        base = Path("data")
    return base / "enterprise_platform" / filename
...
def generate_api_key_material() -> tuple[str, str, str]:
    """Return (full_key, prefix, hash). Full key shown once only."""
    raw = f"qf_{secrets.token_urlsafe(32)}"
    prefix = raw[:10]
