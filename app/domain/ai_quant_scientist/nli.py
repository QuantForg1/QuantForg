...
def answer_question(question: str, *, pack: dict[str, Any]) -> dict[str, Any]:
    ...
    if not q:
        return _base(
            "Ask a research question about regimes, PF, drawdown, or experiments.", []
        )
    ...
    if "replay" in q or ("experiment" in q and "best" in q):
        ...
        if best:
            return _base(
                ...
                f"(PF={best.get('profit_factor')}, composite={best.get('composite_score')}).",  # noqa: E501
    ...
    if "pf" in q or "profit factor" in q:
        return _base(
            f"Production vs candidate PF delta: {comparison.get('profit_factor_difference_pct')}%.",  # noqa: E501
    ...
    if "pattern" in q:
        return _base(
            "Discovered patterns: "
            + "; ".join(p.get("title", "") for p in patterns[:5]),
            patterns[:5],
        )
    ...
    if any(
        k in q
        for k in ("simulation", "ise", "monte carlo", "walk forward", "digital twin")
    ):
        try:
