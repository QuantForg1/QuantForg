"""Integration tests — Real Market Intelligence Platform."""

from __future__ import annotations

from pathlib import Path
from typing import Any

import pytest

from app.application.services.real_market_intelligence_platform import (
    RealMarketIntelligencePlatformService,
)

pytestmark = [pytest.mark.integration]


def _collect_paths(routes: list[Any]) -> list[str]:
    out: list[str] = []
    for route in routes:
        path = getattr(route, "path", None)
        if path:
            out.append(str(path))
        original = getattr(route, "original_router", None)
        ctx = getattr(route, "include_context", None)
        if original is not None:
            prefix = str(getattr(ctx, "prefix", "") or "")
            for child in getattr(original, "routes", []) or []:
                child_path = getattr(child, "path", None)
                if child_path:
                    out.append(f"{prefix}{child_path}")
            continue
        nested = getattr(route, "routes", None)
        if nested:
            out.extend(_collect_paths(list(nested)))
    return out


def test_rmip_router_registered(app) -> None:
    from app.main import _ROUTER_SPECS

    assert (
        "real_market_intelligence_platform",
        "app.presentation.routers.real_market_intelligence_platform",
    ) in _ROUTER_SPECS
    paths = _collect_paths(list(app.routes))
    assert any("real-market-intelligence-platform" in p for p in paths)
    assert any(
        p.endswith("/evaluate") and "real-market-intelligence-platform" in p
        for p in paths
    )
    assert any("/context/current" in p for p in paths)


def test_rmip_domain_no_order_send() -> None:
    root = (
        Path(__file__).resolve().parents[2]
        / "app"
        / "domain"
        / "real_market_intelligence_platform"
    )
    offenders = [
        p.name
        for p in root.glob("*.py")
        if "order_send(" in p.read_text(encoding="utf-8")
        or ".order_send" in p.read_text(encoding="utf-8")
    ]
    assert offenders == []


def test_rmip_service_context_only() -> None:
    out = RealMarketIntelligencePlatformService().evaluate(
        {
            "session_hint": "tokyo",
            "volatility_observations": {"atr": 4, "level": "normal"},
        }
    )
    assert out["context_only"] is True
    assert out["never_order_send"] is True
    assert out["never_change_trading_rules"] is True
    assert out["modifies_decision_engine"] is False
