...
/**
 * Recommendation — Evidence first, actions second, narrative last.
  ...
  return (
    ...
    >
      ...
      <dl className="min-h-0 flex-1 space-y-2 overflow-y-auto text-[11px]">
          ...
          {recommendation.evidence.length === 0 ? (
            ...
            <ul className="space-y-0.5 text-[var(--fg-muted)]">
              {recommendation.evidence.map((e) => (
                <li key={e.slice(0, 40)} className="flex gap-1.5">
                  <span className="mt-1.5 h-1 w-1 shrink-0 rounded-full bg-[var(--fg-subtle)]" aria-hidden />
                  <span>{e}</span>
                </li>
              ))}
            </ul>
        ...
        <p className="qf-caption pt-1 text-[var(--fg-subtle)]">
...
function Field({
  ...
  return (
    <div>
      <dt className="text-[10px] uppercase tracking-[0.06em] text-[var(--fg-subtle)]">
        {label}
      </dt>
