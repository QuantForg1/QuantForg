...
/**
 * Context Lens — market / MTF / session evidence from DE + intelligence.
 */
  ...
  return (
    ...
    >
      <header className="mb-2 shrink-0">
        ...
        <p className="qf-caption text-[var(--fg-subtle)]">Evidence · live analysis fields only</p>
      </header>
      {rows.length === 0 ? (
        <CounselEmpty
          title="No context"
        ...
        <dl className="min-h-0 flex-1 space-y-0 overflow-y-auto text-[11px]">
          {rows.map((r) => (
            <div
              key={r.label}
              className="flex justify-between gap-3 border-b border-[var(--border)]/70 py-1 transition-colors duration-[var(--duration-fast)] hover:bg-[var(--surface-2)]/40"
            >
              <dt className="shrink-0 text-[10px] uppercase tracking-[0.04em] text-[var(--fg-subtle)]">
                {r.label}
              </dt>
              <dd className="text-right tabular text-[var(--fg)]">{r.value}</dd>
            </div>
          ))}
        </dl>
      )}
