...
export function CommandPalette({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-start justify-center bg-black/50 p-4 pt-[12vh] backdrop-blur-sm">
      ...
      />
      <Command className="relative z-10 w-full max-w-xl overflow-hidden rounded-xl border border-[var(--border)] bg-[var(--surface)] shadow-2xl">
        <Command.Input
          value={query}
          onValueChange={setQuery}
          placeholder="Jump to a page or action…"
          className="h-12 w-full border-b border-[var(--border)] bg-transparent px-4 text-sm text-[var(--fg)] outline-none placeholder:text-[var(--fg-subtle)]"
        />
